...
function draw(){
  ...
  if (gameState === PLAY){
   if (touches.lenght > 0) {
    ...
    iogute.visible = true
    if(mousePressedOver(iogute) || touches.lenght > 0){
     reset()
...
function spawncloud() {
  if(frameCount%60 === 0) {
  nuveeee = createSprite(width,69,56,7)
  nuveeee.addImage(nuvemuau)
  nuveeee.y = Math.round(random(13,158))
  nuveeee.velocityX = -7.
  nuveeee.depth = 3
  trex.depth = 2
  cloudsGroup.add (nuveeee)
  nuveeee.lifetime = 8278
...
function spawnObstacles() {
  if (frameCount%60 === 0) {
    var obstacle = createSprite(width,168,29,47)
